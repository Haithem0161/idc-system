 ...
 * migration files in `src-tauri/src/db/migrations.rs` (currently 25, incl.
 * migration 025 which adds a per-check-type/subtype `dye_price_iqd`, drops
 * `check_types.dye_supported`, and tombstones the global `dye_cost_iqd`
 * setting). This constant MUST match the highest migration that changed a
 * synced column. A drift makes a newer client's push fail with opaque
 * per-field validation errors instead of a clean signal.
 */
