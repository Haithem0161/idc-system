      ...
      <DetailSection title={t("accounting.visits.breakdown.title", { defaultValue: "Visit" })}>
        <dl className="overflow-hidden rounded-lg border border-line bg-surface">
          ...
          <Field
            ...
            last={!row.report && row.amount_paid_override_iqd == null}
          />
          {row.report ? (
            <Field
              label={t("accounting.visits.col.report_amount", { defaultValue: "Report (IQD)" })}
              value={
                <span className="font-mono tabular-nums">
                  {formatIqd(row.report_amount_iqd, { locale })}
                </span>
              }
