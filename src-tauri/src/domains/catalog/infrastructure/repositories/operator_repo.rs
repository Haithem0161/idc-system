//! SQLite implementation of `OperatorRepo`.
...
use crate::domains::catalog::domain::repositories::{CatalogListFilter, OperatorRepo};
...
#[async_trait]
impl OperatorRepo for SqliteOperatorRepo {
    async fn upsert(&self, tx: &mut Tx<'_>, op: &Operator) -> AppResult<()> {
        ...
        .bind(&op.name)
        .bind(op.phone.as_deref())
        .bind(op.base_cut_per_check_iqd)
        .bind(op.is_active as i64)
        .bind(op.notes.as_deref())
        .bind(dt_to_str(op.created_at))
        .bind(dt_to_str(op.updated_at))
        .bind(dt_opt_to_str(op.deleted_at))
        .bind(op.version)
        .bind(op.dirty as i64)
        .bind(dt_opt_to_str(op.last_synced_at))
        .bind(op.origin_device_id.as_deref())
        .bind(&op.entity_id)
        .execute(&mut **tx)
        .await?;
        Ok(())
    ...
    async fn get_by_id(&self, id: Uuid) -> AppResult<Option<Operator>> {
        let row: Option<OperatorRow> =
            sqlx::query_as::<_, OperatorRow>("SELECT * FROM operators WHERE id = ?")
                .bind(id.to_string())
    ...
    async fn list(&self, filter: CatalogListFilter) -> AppResult<Vec<Operator>> {
        ...
        let mut q = sqlx::query_as::<_, OperatorRow>(&sql).bind(&filter.entity_id);
...
impl OperatorRow {
    fn into_domain(self) -> AppResult<Operator> {
        Ok(Operator {
            id: parse_uuid(&self.id)?,
            name: self.name,
            phone: self.phone,
            base_cut_per_check_iqd: self.base_cut_per_check_iqd,
