...
pub fn build_thermal(visit: &Visit, snap: &VisitSnapshots, opts: &ReceiptRenderOptions) -> String {
    ...
    if snap.report_cost_iqd > 0 {
        pushln(
            &mut out,
            &pad_columns("Report", &fmt_amount(snap.report_cost_iqd, opts), width),
        );
    }
    sep(&mut out);
    pushln(
...
pub fn build_a5(visit: &Visit, snap: &VisitSnapshots, opts: &ReceiptRenderOptions) -> String {
    ...
    pushln(
        ...
        &pad_columns(
            "Report cost",
            &fmt_amount(snap.report_cost_iqd, opts),
            width,
        ),
    );
    pushln(&mut out, &"-".repeat(width));
    ...
    pushln(
        ...
        &pad_columns(
            "Operator cut",
            &fmt_amount(snap.operator_cut_iqd, opts),
            width,
        ),
    );
    let doctor_label = if snap.internal_pct.is_some() {
        format!("Internal doctor cut ({}%)", snap.internal_pct.unwrap_or(0))
    } else {
        "Doctor cut".into()
    };
    pushln(
        &mut out,
        &pad_columns(&doctor_label, &fmt_amount(snap.doctor_cut_iqd, opts), width),
    );
    pushln(&mut out, "");
